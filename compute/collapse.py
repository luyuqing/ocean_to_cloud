def cal_collaps(steel_diameter,
                corrosion_allowance,
                fabrication_method,
                pipe_material,
    ...
    def f_collaps(x): #define the equation for collpase, see eqn 5.10-5.14 of OS-F101  
        pc = (maximum_external_pressure - min_interal_pressure)* material_factor * safety_class_resistance_factor_others
        def pel(x):
            return (2*young_modulous*(x/steel_diameter)**3)/(1-possoin_ratio**2)
        def pp(x):
            return char_yield_stress*fabrication_factor*2*x/steel_diameter
        return (pc-pel(x))*(pc**2-pp(x)**2)-pc*pel(x)*pp(x)*max_pipe_ovaliaty*steel_diameter/x
                

    def pel_test(x):
        return (2*young_modulous*(x/steel_diameter)**3)/(1-possoin_ratio**2)

    x0 = steel_diameter/20  #x0 = steel_diameter/20.0 # assume the guess value is pipe OD/20
    
    min_wt_collaps_wo_tol = float(fsolve(f_collaps,x0))

    if fabrication_method == 'SMLS':
        min_wt_collaps = (min_wt_collaps_wo_tol + corrosion_allowance)/(1.0 - tol)
    if fabrication_method == 'HFW':
        min_wt_collaps = min_wt_collaps_wo_tol + tol + corrosion_allowance
    if fabrication_method == 'MWP':
        min_wt_collaps = min_wt_collaps_wo_tol + tol + corrosion_allowance

    return min_wt_collaps

# print(cal_collaps(steel_diameter=273.1,
